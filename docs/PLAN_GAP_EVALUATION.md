...
9. **MCP Muninn transport reliability intermittency (partially mitigated, monitoring remains):** framing + parser resilience + queue/backoff + soak harness + tools/call deadline-budget controls + startup-recovery budget gating + host-timeout-derived budgeting are now in-code; remaining risk is host-side environment variability outside wrapper process control (monitor and tune `MUNINN_MCP_TOOL_CALL_DEADLINE_SEC`, `MUNINN_MCP_HOST_TOOLS_CALL_TIMEOUT_SEC`, `MUNINN_MCP_TOOL_CALL_DEADLINE_MARGIN_SEC`, and `MUNINN_MCP_STARTUP_RECOVERY_MIN_BUDGET_SEC` as needed). Latest external evidence: `muninn/add_memory` still hit host `tools/call` 120s timeout in-session on 2026-02-15, indicating rollout/restart validation is still required in the host MCP runtime.
