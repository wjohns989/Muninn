...
class SQLiteMetadataStore:
    ...
    def _row_to_record(self, row: sqlite3.Row) -> MemoryRecord:
        ...
        d["metadata"] = {} if not meta_str or meta_str == "{}" else json.loads(meta_str)
        try:
    ...
    def get_profile_policy_events(self, *, limit: int = 25) -> List[Dict[str, Any]]:
        ...
        for row in rows:
            try:
                updates_str = row["updates_json"]
                updates = {} if not updates_str or updates_str == "{}" else json.loads(updates_str)
            except json.JSONDecodeError:
                updates = {}
            try:
                policy_str = row["policy_json"]
                policy = {} if not policy_str or policy_str == "{}" else json.loads(policy_str)
            except json.JSONDecodeError:
                policy = {}
            events.append(
                {
    ...
    def get_project_goal(
        ...
        *,
        ...
        constraints = []
        embedding = None
        constraints_str = row["constraints_json"]
        if constraints_str and constraints_str != "[]":
            try:
        ...
        embedding_str = row["goal_embedding_json"]
    ...
    def get_user_profile(self, *, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        if raw_profile and raw_profile != "{}":
            try:
