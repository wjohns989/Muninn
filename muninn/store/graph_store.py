...
class GraphStore:
    ...
    def __init__(self, db_path):
        ...
        self._connections: weakref.WeakValueDictionary[int, kuzu.Connection] = (
    ...
    def _get_conn(self) -> kuzu.Connection:
        if not hasattr(self._thread_local, "conn"):
            connection = kuzu.Connection(self._get_db())
    ...
    def close(self):
        # Kuzu requires every QueryResult/Connection to close before Database.
        # Track connections created on worker threads so shutdown releases all
        # mappings and file handles rather than only the lifespan thread's one.
        current_connection = getattr(self._thread_local, "conn", None)
        with self._connections_lock:
            if self._closed:
                return
            self._closed = True
            if current_connection is not None:
                self._connections.setdefault(id(current_connection), current_connection)
