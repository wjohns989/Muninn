...
class GraphStore:
    ...
    def delete_memories_batch(self, memory_ids: List[str]) -> bool:
        """Remove all graph references for a batch of memories."""
        if not memory_ids:
            return True
        conn = self._get_conn()
        try:
            conn.execute("MATCH (m:Memory) WHERE m.id IN $ids DETACH DELETE m", {"ids": memory_ids})
            return True
        except Exception as e:
            logger.debug(f"Delete batch memory references: {e}")
            return False
